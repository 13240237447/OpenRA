...
namespace Util
{
    internal class TickTask
    {
        ...
        public float PassSecond => (float)(PassTime / 1000);
    ...
    internal class TickTimer
    {
        ...
        public int AddDelayTask(float delayTime,Action taskCallback, int loopCount = 1)
        {
            int id = 0;
            id = AddTask((int)(delayTime * 1000), 0, (tickId) =>
            {
                if (tickId == id)
                {
                    taskCallback?.Invoke();
        ...
        /// <param name="loopCount">执行次数</param>
        /// <returns>添加事件成功后返回的ID</returns>
        public int AddTask(int intervalTime, float delayTime, Action<int> taskCallback, Action<int> cancelCallback, int loopCount = 1)
        {
            ...
            {
                Debug.LogError("添加tick任务失败，请告知程序");
                return -1;
            }
            return tid;
        }
        ...
        public bool RemoveTask(int taskId,Action<int> callback)
        {
            ...
            if (callback != null)
            {
                Debug.Assert(task.TaskCallback == callback);
            }
        ...
        {
            foreach (KeyValuePair<int,TickTask> keyValuePair in taskDict)
            {
        ...
        public TickTask GtTask(int taskId)
        {
        ...
        public void TickRefresh()
        {
            // try
            {
                ...
                if (currTickTime == 0)
                {
                    currTickTime = nowTime;
                }
                double subTime = nowTime - currTickTime;
                currTickTime = nowTime;
                foreach (var task in taskDict.Values)
                {
                    ...
                    task.PassTime += subTime;
                    if (nowTime < task.DestTime)
                        continue;
                    ++task.LoopIndex;
                    //循环次数++，若循环idx比循环总数小，则进入下次循环；
                    if (task.LoopIndex < task.LoopCount || task.LoopCount == -1)
                    {
                    ...
                    else
                    {
                        ...
                        //线程安全字典，遍历过程中删除无影响。
                        task.TaskCallback?.Invoke(task.TaskId);
                        RemoveTask(task.TaskId, task.TaskCallback);
                    }
                }
            }
            // catch (Exception e)
